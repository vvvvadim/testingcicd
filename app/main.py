...
COFFEE_URL = "https://random-data-api.com/api/coffee/random_coffee?size=10?response_type=json"
...
def get_random_data():
    ...
    time.sleep(2)
    coffee_list = [Coffee(title=c['blend_name'], origin=c['origin'],intensifier=c['intensifier'],notes=c['notes'].split(', ')) for c in coffee]
    users_list = []
    for num,u in enumerate(names):
        users_list.append(
            User(name=u['first_name'], has_sale=randint(0,1),address=address[num],coffee_id=randint(1,10))
        )
...
def add_user():
    data = request.json
    user = User(name=data['name'], address=data['address'], coffee_id=int(data['coffee_id']), has_sale=int(data['has_sale']))
    with Session(engine) as session_db:
        session_db.add(user)
        session_db.commit()
        session_db.close()
...
def coffee_by_name():
    ...
    print(coffee_name)
    with Session(engine) as session_db:
        coffees = session_db.query(Coffee).filter(Coffee.title.ilike(f'%{coffee_name}%')).all()
        session_db.close()
...
def unique_notes():
    with Session(engine) as session_db:
        unique_notes = session_db.query(func.unnest(Coffee.notes).label("note")).distinct().all()
        session_db.close()
    data = dict()
...
def users_by_country():
    ...
    with Session(engine) as session_db:
        users = session_db.query(User).filter(text("address ->> 'country' = :country")).params(country=country).all()
        session_db.close()
